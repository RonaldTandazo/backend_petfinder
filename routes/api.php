<?php

require __DIR__ . '/Auth/AuthRoutes.php';
require __DIR__ . '/Account/AccountRoutes.php';
require __DIR__ . '/Catalog/CatalogRoutes.php';
require __DIR__ . '/Pet/PetRoutes.php';
require __DIR__ . '/LostPet/LostPetRoutes.php';
require __DIR__ . '/LostPet/LostPetEventRoutes.php';
require __DIR__ . '/Adoption/AdoptionRoutes.php';
require __DIR__ . '/Webhook/WebhookRoutes.php';
require __DIR__ . '/Storage/StorageRoutes.php';
require __DIR__ . '/Picture/PictureRoutes.php';
require __DIR__ . '/Community/CommunityRoutes.php';